...
task VariantFilter {
    ...
    command <<<
        /usr/gitc/gatk4/gatk-launch VariantFiltration -R ~{referenceFasta} -V ~{snpFile} --mask ~{snphcFile} --maskName "processSomatic" --filterNotInMask -O ~{tumor_sample}_Varscan_Gatk_filtered.snp.Somatic.hc.vcf
        /usr/gitc/gatk4/gatk-launch VariantFiltration -R ~{referenceFasta} -V ~{indelFile} --mask ~{indelhcFile} --maskName "processSomatic" --filterNotInMask -O ~{tumor_sample}_Varscan_Gatk_filtered.indel.Somatic.hc.vcf
    >>>
